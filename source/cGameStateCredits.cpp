  ...
  for(int i = 0; i < numberLines; ++i){
    ...
    if(cfr.GetNumberOfTermsFromLine(i) > 0){
      ...
      if(lineType == "sprite" && cfr.GetNumberOfTermsFromLine(i) == 3){
        ...
        m_msg.push_back(SetGraphic(atoi(p2.c_str()), atoi(p3.c_str())));
      }
      ...
      //draw text
        ...
        m_msg.push_back(SetMessage(atoi(p2.c_str()), atoi(p3.c_str()), 
...
void CGameStateCredits::Render(CGraphics &con,  CGameData &data, CConfigData &cfg){
...
::ShowCursor(false);
  ...
  //GRAPHIC_IMAGE gi;
  //gi = con.GetSprite(2);
  //con.RenderGraphicModulate(gi, cfg.ScreenWidth - 240, 40, 255, 255, 255);
