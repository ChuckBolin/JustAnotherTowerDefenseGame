...
void CUserInterface::Render(CGraphics &con,  CGameData &data){
  ...
  con.RenderGraphicModulate(gi, 0, 623, 255, 255, 255);
  ...
  for(int i = 0; i < 6; ++i){
   ...
   // }
    ...
    if(data.m_credits < cost){
      gi = con.GetSprite(1600 + i);
      con.RenderGraphicModulate(gi, 312 + (i * 50), 676, 255, 255, 255);
    }
  }

   //   gi = con.GetSprite(1605);
   //   con.RenderGraphicModulate(gi, 562, 676, 255, 255, 255);

  //pause graphic
  if(data.m_bPaused == true){
    gi = con.GetSprite(1502);
    con.RenderGraphicModulate(gi, 385, 643, 255, 255, 255);
  }

  //render x1 x2 time scale
  if(data.m_timeScale == 1){
    gi = con.GetSprite(1506);
    con.RenderGraphicModulate(gi, 457, 641, 255, 255, 255);
    gi = con.GetSprite(1507);
    con.RenderGraphicModulate(gi, 499, 645, 255, 255, 255);
  }
  if(data.m_timeScale == 2){
    gi = con.GetSprite(1503);
    con.RenderGraphicModulate(gi, 460, 643, 255, 255, 255);
    gi = con.GetSprite(1504);
    con.RenderGraphicModulate(gi, 496, 641, 255, 255, 255);
  } 

  //render yellow outline around selected defensive unit
  if(m_UIstate >= 7 && m_UIstate <= 14){
    gi = con.GetSprite(1505);
    con.RenderGraphicModulate(gi, 313 + (50 * (m_UIstate - 7)), 675, 255, 255, 255);
  }
  ...
  for(int i = 0; i < sScore.size();++i){
    for(int j = 0; j < sRef.size(); ++j){
      if(sScore.at(i) == sRef.at(j)){
        ...
        con.RenderGraphicModulate(gi, posX, posY, 255, 255, 255);
        posX += gi.width;
        break;
      }
    }
  ...
  //renders credits  
  //*******************************************
  posX = 856;//m_panelS.x1 + 17;
  posY = 733;//m_panelS.y1 + 10;
  sRef = "0123456789";
  oss.str("");
  oss << data.m_credits;
  sNum = oss.str();
  ...
  if(sNum.length() < 6)
    sPrefix.append(6 - sNum.length(), '0');
  std::string sCredits = sPrefix + sNum;

  for(int i = 0; i < sCredits.size();++i){
    for(int j = 0; j < sRef.size(); ++j){
      if(sCredits.at(i) == sRef.at(j)){
        gi = con.GetSprite(1200 + j);
        con.RenderGraphicModulate(gi, posX, posY, 255, 255, 255);
        posX += gi.width;
        break;
  ...
  sRef = "0123456789";
  oss.str("");
  oss << data.m_waveGen.GetWaveCount();
  sNum = oss.str();
  sPrefix = "";
  if(sNum.length() < 3)
    sPrefix.append(3 - sNum.length(), '0');
  std::string sWavecount = sPrefix + sNum;

  for(int i = 0; i < sWavecount.size();++i){
    for(int j = 0; j < sRef.size(); ++j){
      if(sWavecount.at(i) == sRef.at(j)){
        gi = con.GetSprite(1200 + j);
        con.RenderGraphicModulate(gi, posX, posY, 255, 255, 255);
        posX += gi.width;
        break;
  ...
  //repair and upgrade sprite
  gi = con.GetSprite(1232);
  con.RenderGraphicModulate(gi, 719, 684, 255, 255, 255);
  ...
  sRef = "0123456789";
  oss.str("");
  oss << data.m_costRepair;
  sNum = oss.str();
  sPrefix = "";
  //if(sNum.length() < 3)
  //  sPrefix.append(3 - sNum.length(), '0');
  std::string sRepair = sPrefix + sNum;

  for(int i = 0; i < sRepair.size();++i){
    for(int j = 0; j < sRef.size(); ++j){
      if(sRepair.at(i) == sRef.at(j)){
        gi = con.GetSprite(1200 + j);
        con.RenderGraphicModulate(gi, posX, posY, 255, 255, 255);
        posX += gi.width;
        break;
  ...
  sRef = "0123456789";
  oss.str("");
  oss << data.m_costUpgrade;
  sNum = oss.str();
  sPrefix = "";
  //if(sNum.length() < 3)
  //  sPrefix.append(3 - sNum.length(), '0');
  std::string sUpgrade = sPrefix + sNum;

  for(int i = 0; i < sUpgrade.size();++i){
    for(int j = 0; j < sRef.size(); ++j){
      if(sUpgrade.at(i) == sRef.at(j)){
        gi = con.GetSprite(1200 + j);
        con.RenderGraphicModulate(gi, posX, posY, 255, 255, 255);
        posX += gi.width;
        break;
  ...
  //con.RenderGraphicModulate(gi, 960, 728, 255, 255, 255);
  if(data.m_playSound == true)
    gi = con.GetSprite(1513);
  else
    gi = con.GetSprite(1512);
  con.RenderGraphicModulate(gi, m_sound.x1, m_sound.y1, 255, 255, 255);
